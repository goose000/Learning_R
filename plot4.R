...
par(mfcol=c(2,2))
#add the GAP plot
with(graph_data,plot(Time,Global_active_power,ylab="Global Active Power (kilowatts)",xlab="",type="n"))
with(graph_data,lines(Time,Global_active_power))

#add the sub_meter plot
with(graph_data,plot(Time,Sub_metering_1,ylab="Energy Sub Metering",xlab="",type="n"))
...
with(graph_data,plot(Time,Voltage,xlab="datetime",type="n"))
with(graph_data,lines(Time,Voltage))

#add the GRP Plot
with(graph_data,plot(Time,Global_reactive_power,xlab="datetime",type="n"))
with(graph_data,lines(Time,Global_reactive_power))
dev.off()
